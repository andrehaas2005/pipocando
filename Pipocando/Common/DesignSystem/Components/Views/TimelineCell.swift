...
class TimelineCell: UITableViewCell {
  ...
  override func prepareForReuse() {
    ...
    dotView.subviews.forEach { $0.removeFromSuperview() }
  ...
  func configure(_ serie: Serie, isFirst: Bool, isFavorite: Bool) {
    chapterView.configure(with: serie, isFavorite: isFavorite)
    timeLabel.text = serie.firstAirDate
    if isFirst {
